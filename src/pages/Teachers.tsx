...
function getCopy(locale: 'ar' | 'en') {
  return locale === 'ar'
    ? {
        loadError: 'فشل تحميل المعلمين',
        saveError: 'فشل حفظ المعلم',
        deleteError: 'فشل حذف المعلم',
...
export default function Teachers() {
  ...
  const stats = [
    ...
    { label: copy.totalClasses, value: teachers.reduce((s, t) => s + t.classes, 0), color: '#a78bfa' },
    { label: copy.avgClasses, value: teachers.length ? Math.round(teachers.reduce((s, t) => s + t.classes, 0) / teachers.length) : 0, color: '#60a5fa' },
  ]
      ...
      <Modal
        ...
        footer={
          <div className="flex gap-2.5">
            <button onClick={() => setShowModal(false)} className="flex-1 py-2.5 text-[13px] btn-ghost rounded-xl">{copy.cancel}</button>
        ...
        <div className="flex flex-col gap-4">
          {FIELDS.map(f => (
            <div key={f.key} className="flex flex-col gap-1.5">
              ...
              <input value={form[f.key]} onChange={e => setForm({ ...form, [f.key]: e.target.value })} placeholder={f.placeholder} className="input" />
            </div>
          ))}
          <div className="flex flex-col gap-1.5">
            <label className="text-[12px] font-semibold" style={{ color: 'var(--text-muted)' }}>{copy.numberOfClasses}</label>
            <input type="number" value={form.classes} onChange={e => setForm({ ...form, classes: e.target.value })} placeholder="0" className="input" min="0" />
          </div>
        </div>
      </Modal>
            ...
            <div key={t.id} className="card card-glow p-5 group" style={{ transition: 'all .25s' }}>
              ...
              <div className="flex items-center justify-between pt-4" style={{ borderTop: '1px solid var(--border)' }}>
                <div className="flex items-center gap-1.5" style={{ color: 'var(--text-muted)' }}>
                  ...
                  <span className="text-[12px]">{t.classes} {copy.classes}</span>
                </div>
                {t.phone && (
                  <div className="flex items-center gap-1.5" style={{ color: 'var(--text-muted)' }}>
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
