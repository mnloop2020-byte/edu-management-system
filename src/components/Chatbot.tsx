...
const starterPrompts = ['كم عدد الطلاب؟', 'ملخص المدفوعات', 'من غاب اليوم؟', 'أعطني تقريرًا سريعًا']

export default function Chatbot() {
  const [open, setOpen] = useState(false)
  const [messages, setMessages] = useState<Message[]>([
    { role: 'assistant', content: 'مرحبًا! أنا مساعدك الذكي. أستطيع مساعدتك في الطلاب والحضور والمدفوعات والتقارير. كيف أساعدك اليوم؟' },
  ])
  ...
  async function sendMessage() {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: 'عذرًا، حدث خطأ في الاتصال. حاول مرة أخرى.' }])
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <>
      ...
      {open && (
        <div className="fixed bottom-24 right-6 z-50 w-[360px] rounded-2xl shadow-2xl flex flex-col overflow-hidden" style={{ height: '500px', background: '#111318', border: '1px solid rgba(255,255,255,0.08)' }}>
          <div className="px-4 py-3.5 border-b border-white/[0.06] flex items-center gap-3 shrink-0">
            ...
            <div>
              ...
              <p className="text-[10px] text-white/35">مساعد سريع للتقارير والعمليات اليومية</p>
            </div>
              ...
              <div key={index} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div
                  className={`max-w-[85%] px-3.5 py-2.5 rounded-2xl text-[13px] leading-6 ${
                    message.role === 'user' ? 'bg-violet-600 text-white rounded-br-sm' : 'bg-white/[0.05] text-white/80 rounded-bl-sm'
                  }`}
                  dir="rtl"
                >
                  {message.content}
            ...
            <div className="px-4 pb-2 flex flex-wrap gap-1.5 shrink-0">
              {starterPrompts.map(prompt => (
                <button
                  ...
                  className="text-[11px] px-2.5 py-1 rounded-lg border border-white/[0.08] text-white/50 hover:text-violet-300 hover:border-violet-500/30 transition-colors"
                >
                  {prompt}
                </button>
              ))}
